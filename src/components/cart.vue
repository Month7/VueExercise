<template>
    <div class="checkout">
      <div id="app">
        ...
        <div class="md-modal modal-msg md-modal-transition" id="showModal" :class="{'md-show':currentClassShow}">
          <div class="md-modal-inner">
            <div class="md-top">
              <button class="md-close" @click="closeCurrentDialog">关闭</button>
            </div>
            <div class="md-content">
              ...
              <div class="btn-wrap col-2">
                <button class="btn btn--m" id="btnModalConfirm" @click="removeItem(item)">Yes</button>
                <button class="btn btn--m btn--red" id="btnModalCancel" @click="closeCurrentDialog">No</button>
              </div>
            </div>
          </div>
        </div>
...
export default {
  data(){
        ...
        productList:[
      {
        ...
        "productPrice":19,
        "productQuantity":1,
        "productImage":"../../static/goods-1.jpg",
        "parts":[
          {
            "partsId":"10001",
            "partsName":"打火机",
            "imgSrc":"../../static/part-1.jpg"
          }
        ]
      },
      {
        ...
        "productPrice":8,
        "productQuantity":5,
        "productImage":"../../static/goods-2.jpg",
        "parts":[
          {
            "partsId":"20001",
            "partsName":"吸管",
            "imgSrc":"../../static/part-2.jpg"
          }
        ]
      },
      {
  ...
  methods:{
      ...
      calTotalMoney(){
        ...
        this.productList.forEach(function(val,index){
            if(val.isChecked){
                self.totalMoney += val.productPrice*val.productQuantity;
            }
      ...
      checkAll(flag){
        ...
        this.productList.forEach(function(val,index){
            if(typeof val.isChecked == 'undefined'){
                self.$set(val,'isChecked',self.selectedAll);
            }
            else{
                val.isChecked = self.selectedAll;
            }
        });
        this.calTotalMoney();
      },
      removeConfirm(item){
          this.currentClassShow = true;
      },
      closeCurrentDialog(){
          this.currentClassShow = false;
      },
      removeItem(item){
